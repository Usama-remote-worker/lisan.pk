            ...
            <div className="container mx-auto px-4 max-w-7xl">
                ...
                <div className="grid md:grid-cols-3 gap-8 mb-16 font-sans">
                    ...
                    <div className="bg-white p-8 rounded-[2rem] border border-slate-100 shadow-sm">
                        <h2 className="text-xl font-bold text-slate-900 mb-6 font-serif border-b border-slate-100 pb-4">
                            Core Pages
                        </h2>
                        <ul className="space-y-3 font-sans">
                            <li><Link href="/" className="text-sm font-semibold text-slate-600 hover:text-emerald-700 flex items-center gap-2">✓ Home Page <span className="text-xs text-slate-400 font-normal font-mono">(/)</span></Link></li>
                            <li><Link href="/services" className="text-sm font-semibold text-slate-600 hover:text-emerald-700 flex items-center gap-2">✓ All Services <span className="text-xs text-slate-400 font-normal font-mono">(/services)</span></Link></li>
                            <li><Link href="/consultancy/saudi-scholarship" className="text-sm font-semibold text-slate-600 hover:text-emerald-700 flex items-center gap-2">✓ Saudi Scholarships <span className="text-xs text-slate-400 font-normal font-mono">(/consultancy/saudi-scholarship)</span></Link></li>
                            <li><Link href="/blog" className="text-sm font-semibold text-slate-600 hover:text-emerald-700 flex items-center gap-2">✓ News & Blog <span className="text-xs text-slate-400 font-normal font-mono">(/blog)</span></Link></li>
                            <li><Link href="/about" className="text-sm font-semibold text-slate-600 hover:text-emerald-700 flex items-center gap-2">✓ About Lisan.pk <span className="text-xs text-slate-400 font-normal font-mono">(/about)</span></Link></li>
                            <li><Link href="/contact" className="text-sm font-semibold text-slate-600 hover:text-emerald-700 flex items-center gap-2">✓ Get in Touch <span className="text-xs text-slate-400 font-normal font-mono">(/contact)</span></Link></li>
                        </ul>
                    </div>

                    {/* University Guides Card */}
                    <div className="bg-white p-8 rounded-[2rem] border border-slate-100 shadow-sm font-sans">
                        <h2 className="text-xl font-bold text-slate-900 mb-6 font-serif border-b border-slate-100 pb-4">
                            University Guides
                        </h2>
                        <ul className="space-y-3 font-sans">
                    ...
                    <div className="bg-white p-8 rounded-[2rem] border border-slate-100 shadow-sm font-sans">
                        ...
                        <ul className="space-y-3 font-sans">
                            ...
                            {serviceEntries.map(([slug, srv]) => (
                                <li key={slug} className="pl-4 border-l border-slate-100 font-sans">
                ...
                <div className="bg-white p-8 rounded-[2rem] border border-slate-100 shadow-sm mb-16 font-sans">
                    <h2 className="text-xl font-bold text-slate-900 mb-6 font-serif border-b border-slate-100 pb-4">
                        Articles & Resources Directory
                    </h2>
                    <div className="grid sm:grid-cols-2 gap-4 font-sans">
                        {posts.map((post) => (
                            <Link key={post.slug} href={`/blog/${post.slug}`} className="p-4 rounded-xl hover:bg-slate-50 border border-transparent hover:border-slate-100 flex items-center justify-between group transition-all text-sm">
                ...
                <div className="bg-white p-8 md:p-12 rounded-[3rem] border border-slate-100 shadow-xl space-y-12 font-sans">
                    <div className="border-b border-slate-100 pb-6 font-sans">
                        <h2 className="text-2xl md:text-3xl font-bold text-slate-900 mb-3 font-serif">
                            Localized Translation Directory (29 Cities of Pakistan)
                        </h2>
                                    ...
                                    {citySlugs.map((slug) => {
                                        ...
                                        return (
                                            <div key={slug} className="p-6 bg-slate-50/50 rounded-2xl border border-slate-200/60 hover:border-emerald-200 transition-colors">
                                                <Link href={`/locations/${slug}`} className="font-bold text-slate-900 hover:text-emerald-700 text-base flex items-center gap-2 group mb-3 font-serif">
                                                    <span className="w-2 h-2 rounded-full bg-emerald-500"></span>
                                                    {cityData.name} Branch
                                                    <span className="opacity-0 group-hover:opacity-100 group-hover:translate-x-1 transition-all text-emerald-600 font-bold">→</span>
                                                </Link>
                                                <p className="text-xs text-slate-400 mb-4 line-clamp-1 italic">
                                                    Serving {cityData.landmark}
                                                </p>
                                                {/* In-City Direct Service Sublinks */}
                                                <div className="flex flex-wrap gap-x-2 gap-y-1.5 border-t border-slate-100 pt-3 font-sans">
